...
Page({
    ...
    submit: function() {
        var i = this,
            r = i.data.detail;
        i.data.posting || ("" != r.realname && r.realname ? "" != r.mobile && r.mobile ? "" != r.city && r.city ? !(i.data.street.length > 0) || "" != r.street && r.street ? "" != r.address && r.address ? r.datavalue ? /^[1][3-9]\d{9}$|^([6|9])\d{7}$|^[0][9]\d{8}$|^[6]([8|6])\d{5}$/.test(r.mobile) ? (r.id = i.data.id, i.setData({
            posting: !0
            ...
            i.setData({
                ...
                r.id = s.addressid, "member" == i.data.type && wx.navigateBack(), "quickaddress" == i.data.type ? (t.setCache("orderAddress", r, 30), wx.navigateBack()) : wx.redirectTo({
