...
Page({
    ...
    onReachBottom: function() {
        console.error(this.data.loaded + "  " + this.data.list.length + "  " + this.data.total), this.data.loaded || this.data.list.length == this.data.total || this.get_list()
    },
    get_list: function() {
        var t = this;
        t.setData({
    ...
    submit: function(t) {
        var s = t.currentTarget.dataset,
            r = this; - 1 != s.startbuy && ("0" != s.stock ? a.post("membercard.order.create_order", {
            id: s.id
        }, function(t) {
